  ...
  return (
    <div className="grid min-h-screen place-items-center bg-[#7655aa] px-5">
      <div className="w-full max-w-md rounded-[8px] bg-white p-8 shadow-card">
        ...
        <p className="mt-2 text-sm text-[#746d82]">Nhập mật khẩu mới cho tài khoản quản trị của bạn.</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-4">
          <div>
            <Input type="password" placeholder="Mật khẩu mới" {...register("new_password")} className="h-11 rounded-[8px]" />
            {errors.new_password && <p className="mt-1 text-xs font-bold text-destructive">{errors.new_password.message}</p>}
          </div>
          <div>
            <Input type="password" placeholder="Xác nhận mật khẩu mới" {...register("confirm_password")} className="h-11 rounded-[8px]" />
            {errors.confirm_password && <p className="mt-1 text-xs font-bold text-destructive">{errors.confirm_password.message}</p>}
          </div>
